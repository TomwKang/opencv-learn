...
base64_decode(const uint8_t* text, size_t sz, int* out_size) {
	...
	if(DecodeBuffer!=NULL){
		my_free(DecodeBuffer);
		DecodeBuffer=NULL;
	}
	if (decode_sz > SMALL_CHUNK) {
		DecodeBuffer = (char *) my_malloc(decode_sz);
	}
	else {
		DecodeBuffer = (char *) my_malloc(SMALL_CHUNK);
	}
	int i, j;
	int output = 0;
	for (i = 0; i< (int)sz;) {
		...
		case 0:
			...
			DecodeBuffer[output] = v >> 16;
			DecodeBuffer[output + 1] = (v >> 8) & 0xff;
			DecodeBuffer[output + 2] = v & 0xff;
			output += 3;
			break;
		case 1:
			if (c[3] != -2 || (c[2] & 3) != 0) {
			...
			DecodeBuffer[output] = v >> 8;
			DecodeBuffer[output + 1] = v & 0xff;
			output += 2;
			break;
		case 2:
